// uart_scoreboard.sv — dual-input scoreboard.
// Included from uart_env_pkg.sv (which provides the analysis_imp typedefs).

class uart_scoreboard extends uvm_component;
    `uvm_component_utils(uart_scoreboard)

    // UART register offset of the DATA register (addr[7:2] == 0x00).
    ...
    //   0            SoC level   — no external AXI master to watch.  The
    //                scoreboard just accumulates the serial output into
    //                txd_all so tests can grep for signatures.
    bit match_mode = 1'b1;
    ...
    // SV's string type has no built-in find; provide a tiny helper.
