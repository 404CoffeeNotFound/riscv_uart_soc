// uart_core.sv — 8N1 UART core: baud generator, TX/RX engines, FIFOs,
// register file.  Exposes a PicoRV32-style native bus slave for simplicity;
// uart_top.sv wraps this with an AXI4-Lite slave adapter.
//
...
// ---------------------------------------------------------------------------
// Baud generator — emits 1-cycle pulse every baud_div system clocks (16x oversample).
// ---------------------------------------------------------------------------
            ...
            case (state)
                ...
                R_START: if (tick_x16) begin
                    if (os_cnt == 4'd7) begin
                        if (rxd_s2 == 1'b0) begin
                            os_cnt  <= '0;
                            bit_idx <= '0;
                            state   <= R_DATA;
                        end else state <= R_IDLE;
                    end else os_cnt <= os_cnt + 4'd1;
                end
                R_DATA: if (tick_x16) begin
                    if (os_cnt == 4'd15) begin
                        os_cnt <= '0;
                        shreg  <= {rxd_s2, shreg[7:1]};
...
// ---------------------------------------------------------------------------
// uart_core — UART with native-style bus slave.
// uart_top wraps this with AXI4-Lite.
// ---------------------------------------------------------------------------
module uart_core #(
    ...
    always_ff @(posedge clk) begin
        if (!rst_n) begin
            ...
            if (bus_we && mem_addr[7:2] == 6'h02 && mem_wdata[5])
    ...
    always_ff @(posedge clk) begin
        if (!rst_n) begin
            ...
            case (mem_addr[7:2])
                ...
                6'h03: baud_div_q <= mem_wdata[15:0];
                default: ;
            endcase
        end
    end
